...
class Author {
	...
	public function isLogin() {
		...
		if(!isset($author['name']) || $name !== $author['name']) {
			...
			Session::instance()->set('author', (Array) $author);
		}
		return TRUE;
	}
	...
	public function ldapLogin($name, $password) {
		...
		$mail = $staff[0]['mail'][0];
		$dnArray = explode(',', $dn);
		$cnName= explode('-', $dnArray[0]);
		$givenName = str_replace('CN=', '', $cnName[0]);
		$unit = $dnArray[2];
		$unitArr = explode('=', $unit);
		$departmentNames = explode('-', $unitArr[1] . '-' . trim($cnName[1]));
		$parentId = 0;
		$departmentIds = array();
